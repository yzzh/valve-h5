<template>
    <div id="watchonline" ref="nontheaterRoot" :style="{
        height: contentHeight + 'px',
        overflow: 'auto'
        }">
        <header>{{moviefree.subject_collection.name}}</header>
        <ul class="nontheater-content">
            <listItem
                v-for="(item,index) in moviefree.subject_collection_items"
                :item="item"
                :key="index"
            ></listItem>
        </ul>
...
<script>
    // import { getMovieFree } from '../../../../fetchs/movie/index.js';
            ...
            loadMovieShow(e, forceFlag){
                ...
                if(forceFlag || this.loadStatus === 0 && isLoad){
                    ...
                    .then(res => {
                        ...
                        this.moviefree.subject_collection = {...this.moviefree.subject_collection, ...res.subject_collection};
                        this.moviefree.subject_collection_items = [...this.moviefree.subject_collection_items, ...res.subject_collection_items];
                        this.loadStatus = 0; //数据获取到以后修改加载状态为可以加载
                        this.start += 18;
                        console.log('this.moviefree', this.moviefree);
                    })
                    .catch(err => {
                        console.log('err', err);
                        this.loadStatus = 0;
